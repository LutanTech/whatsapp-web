...
function chatAvatarTarget(row){
    ...
    return jid==="status@broadcast"?clean(row?.sender)||jid:jid
}
...
async function getSavedContactName(session,jid){
    ...
    if(!target)return ""
    try{
        const name=await require("./bot").getContactName(session,target)
        if(name)return clean(name)
    }catch{}
    const normalized=target.replace(/:\d+@/,"@"),phone=normalized.split("@")[0]
    const contacts=session?.contacts
    if(contacts instanceof Map){
        const direct=contacts.get(target)||contacts.get(normalized)||contacts.get(`${phone}@s.whatsapp.net`)
        if(direct?.name)return clean(direct.name)
        const match=Array.from(contacts.values()).find(c=>
            c?.id===target||c?.id===normalized||
            c?.lid===target||c?.lid===normalized||
            c?.pn===target||c?.pn===normalized||
            String(c?.phoneNumber||"")===phone
        )
...
async function refreshRowAvatar(session,conversationKey,row){
    ...
    const senderTarget=clean(row.sender)&&clean(row.sender)!==chatTarget?clean(row.sender):""
    ...
    const savedChatName=await getSavedContactName(session,chatTarget)
    const savedSenderName=await getSavedContactName(session,row.sender)
    const safeChatName=savedChatName&&savedChatName!==ownerName?savedChatName:""
    const safeSenderName=savedSenderName&&
        ...
        ?savedSenderName:""
    const incomingName=clean(row.incoming_name||row.incoming_push_name)
    ...
    const nameWhere=row.jid==="status@broadcast"?" AND sender=?":""
    const params=[chatAvatar,senderAvatar,chatAvatar,session.id,conversationKey]
    if(nameWhere)params.push(row.sender)

    await run(`
        UPDATE messages SET
            avatar=COALESCE(NULLIF(?,''),
            avatar),
            sender_avatar=COALESCE(NULLIF(?,''),
            sender_avatar),
            chat_avatar=COALESCE(NULLIF(?,''),
            chat_avatar)
...
async function refreshConversationAvatar(session,conversationKey){
    if(!session?.id||!session?.sock||!conversationKey)return null

    const rows=await all(`
        SELECT conversation_key,jid,sender,sender_name,push_name,from_me,
               group_name,channel_name,avatar,sender_avatar,chat_avatar
        FROM messages
    ...
    const incomingRows=await all(`
        ...
        FROM messages
        WHERE session_id=? AND conversation_key=? AND from_me=0
        ORDER BY id DESC LIMIT 1
    `,[session.id,conversationKey])

    const incoming=incomingRows[0]||{}
    const ownerName=clean(session.sock?.user?.name)
    const incomingContactName=await getSavedContactName(session,incoming.sender)
    ...
    const statusRows=await all(`
        SELECT m.conversation_key,m.jid,m.sender,m.sender_name,m.push_name,
               m.from_me,m.group_name,m.channel_name,m.avatar,
               m.sender_avatar,m.chat_avatar
        FROM messages m
        INNER JOIN (
            SELECT sender,MAX(id) latest_id
            FROM messages
            WHERE session_id=? AND conversation_key=? AND jid='status@broadcast'
            GROUP BY sender
    ...
    for(const row of statusRows){
        const contactName=await getSavedContactName(session,row.sender)
        refreshed.push(await refreshRowAvatar(session,conversationKey,{
            ...row,
            incoming_name:clean(contactName||row.push_name||row.sender_name)
        }))
    }
...
async function refreshSessionAvatars(session){
    if(!session?.id||!session?.sock)return 0

    const rows=await all(`
        SELECT conversation_key
        FROM messages
        WHERE session_id=? AND conversation_key IS NOT NULL
    ...
    for(const row of rows){
        try{
            ...
            console.warn(`[AVATAR] Refresh failed for ${row.conversation_key}:`,error.message)
        }
    }
...
module.exports={refreshConversationAvatar,refreshSessionAvatars}