...
async function createSession(id, phone = "") {
    ...
    sock.ev.on("presence.update", ({ id, presences }) => {
        console.log("[PRESENCE UPDATE]", {
            id,
            presences: JSON.stringify(presences, null, 2)
        })
    })
    ...
    sock.ev.on("connection.update", async update => {
        ...
        if (connection === "open") {
            ...
            await saveStatus(id, phone, "connected")
            console.log(`[WA] Connected: ${id}`)
        
            const jid = "0106903141@s.whatsapp.net"
            setTimeout(async()=>{
                try{
                    await sock.presenceSubscribe(jid)
                    console.log("[PRESENCE] Subscribed:",jid)
                }catch(e){
            ...
            refreshSessionAvatars(session)
                .then(count => {
                    console.log(`[AVATAR] Refreshed ${count} conversation avatar(s) for ${id}`)
                })
                .catch(error => {
                    console.error(`[AVATAR] Session refresh failed: ${id}`, error.message)
                })
        }
        ...
        let code
        try {
            code = new Boom(lastDisconnect?.error).output.statusCode
        } catch {
            code = undefined
        }

        session.status = "disconnected"
        await saveStatus(id, phone, "disconnected")

        console.error(`[WA] Disconnected: ${id} (${code || "unknown"})`)

        if (lastDisconnect?.error?.message) {
            console.error(`[WA] ${lastDisconnect.error.message}`)
        }

        if (code === DisconnectReason.loggedOut || code === DisconnectReason.badSession) {
            sessions.delete(id)
            console.error(`[WA] Session invalid: ${id}`)
            return
        }

        if (code === DisconnectReason.connectionReplaced) {
            sessions.delete(id)
            console.error(`[WA] Session replaced: ${id}`)
            return
        }
        ...
        if (!reconnectable.includes(code)) {
            sessions.delete(id)
            console.error(`[WA] Not reconnecting: ${id}`)
            return
        }

        sessions.delete(id)
        console.log(`[WA] Reconnecting: ${id}`)

        setTimeout(() => {
            createSession(id, phone).catch(e => {
                console.error(`[WA] Reconnect failed: ${id}`, e.message)
            })
        }, 5000)
    })
    ...
    sock.ev.on("messages.upsert", async event => {
        for (const message of event.messages) {
            try {
                await require("./bot").handleMessage(session, message)
            } catch (e) {
                console.error("[MSG] Processing failed:", e.message)
            }
        }
    })
...
async function restoreSessions() {
    try {
        ...
        console.log(`[AUTH] Restoring ${rows.length} session(s)`)

        for (const row of rows) {
            const folder = path.join(SESSION_DIR, row.id)
            if (!fs.existsSync(folder)) continue

            createSession(row.id, row.phone).catch(e => {
                console.error(`[AUTH] Restore failed: ${row.id}`, e.message)
            })
        }
    } catch (e) {
...
function setSessionEventEmitter(emitter) {
    sessionEventEmitter = typeof emitter === "function" ? emitter : null
}
