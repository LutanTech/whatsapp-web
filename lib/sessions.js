...
async function createSession(id,phone=""){
    ...
    sock.ev.on(
        "presence.update",
        ({id: presenceId, presences})=>{
            const items = presences && typeof presences === "object"
                ? Object.entries(presences)
                : presenceId
                    ? [[presenceId, {}]]
                    : []

            for(const [jid, value] of items){
                const target = normalizeContactJid(jid)
                if(!target) continue

                const state = String(
                    value?.lastKnownPresence ||
                    value?.presence ||
                    value?.status ||
                    "unavailable"
                ).toLowerCase()

                session.presence.set(target, {
                    state,
                    updatedAt: Date.now()
                })
                sessionEventEmitter?.({
                    ...
                    jid: target,
                    presence: state,
                    is_online: isOnlinePresence(state)
                })
            }
        }
    )
